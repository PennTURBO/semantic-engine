...
class DrivetrainSparqlChecks extends ProjectwideGlobals
{
    ...
    def preExpansionChecks (cxn: RepositoryConnection): Boolean =
    {
        ...
        var proceed: Boolean = precheck.checkAllSubjectsHaveAType(cxn)
        if (proceed) proceed = precheck.checkForUnexpectedClasses(cxn)
        if (proceed) proceed = precheck.checkAllObjectsAreLiterals(cxn)
        //if (proceed) proceed = precheck.checkForPropertiesOutOfRange(cxn)
        if (proceed) proceed = precheck.checkForUnexpectedPredicates(cxn)
        if (proceed) proceed = precheck.checkBiosexURIsAreValid(cxn)
        if (proceed) proceed = precheck.checkEncounterRegistryURIsAreValid(cxn)
        if (proceed) proceed = precheck.checkForRequiredParticipantShortcuts(cxn)
        if (proceed) proceed = precheck.checkForRequiredHealthcareEncounterShortcuts(cxn)
        if (proceed) proceed = precheck.checkForRequiredBiobankEncounterShortcuts(cxn)
        if (proceed) proceed = precheck.checkForValidParticipantBirthShortcuts(cxn)
        if (proceed) proceed = precheck.checkForValidParticipantBiosexShortcuts(cxn)
        if (proceed) proceed = precheck.checkForValidHealthcareEncounterDateShortcuts(cxn)
        if (proceed) proceed = precheck.checkForValidBiobankEncounterDateShortcuts(cxn)
        if (proceed) proceed = precheck.checkForValidEncounterDiagnosisShortcuts(cxn)
        if (proceed) proceed = precheck.checkForValidEncounterPrescriptionShortcuts(cxn)
        if (proceed) proceed = precheck.checkForMultipleParticipantDependentNodes(cxn)
        if (proceed) proceed = precheck.checkForMultipleBiobankEncounterDependentNodes(cxn)
        if (proceed) proceed = precheck.checkForMultipleHealthcareEncounterDependentNodes(cxn)
        if (proceed) proceed = precheck.checkForMultiplePrescriptionDependentNodes(cxn)
        if (proceed) proceed = precheck.checkForMultipleDiagnosisDependentNodes(cxn)
