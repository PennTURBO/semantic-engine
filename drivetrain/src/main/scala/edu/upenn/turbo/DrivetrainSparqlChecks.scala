...
class DrivetrainSparqlChecks extends ProjectwideGlobals
{
    ...
    def preExpansionChecks (cxn: RepositoryConnection): Boolean =
    {
        ...
        if (proceed) proceed = precheck.checkForRequiredHealthcareEncounterShortcuts(cxn, graphsString)
        if (proceed) proceed = precheck.checkForRequiredBiobankEncounterShortcuts(cxn, graphsString)
        if (proceed) proceed = precheck.checkForValidParticipantBirthShortcuts(cxn, graphsString)
        if (proceed) proceed = precheck.checkForValidParticipantBiosexShortcuts(cxn, graphsString)
        if (proceed) proceed = precheck.checkForValidHealthcareEncounterDateShortcuts(cxn, graphsString)
        if (proceed) proceed = precheck.checkForValidBiobankEncounterDateShortcuts(cxn, graphsString)
