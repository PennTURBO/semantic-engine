...
{   
    ...
    def runMedicationMapping(cxn: RepositoryConnection): Boolean =
    {
        ...
        {
            //val fr: FileReader = new FileReader("C://Users//Hayden_Freedman//turbo//utilities//r//med_map_needs_tidying//just_meds_short.txt")
            /*val fr: FileReader = new FileReader("../medmap/just_meds.txt")
            val br: BufferedReader = new BufferedReader (fr)*/
            var medList: ArrayBuffer[String] = 
              ArrayBuffer("RYTHMOL 150 MG OR TABS", 
                  "ATENOLOL 25 MG OR TABS", 
                  "CARDIZEM LA 180 MG OR TAB SR 24HR", 
                  "PHARMACY MED RECONCILIATION CONSULT FOR DC", 
                  "OXYCODONE-ACETAMINOPHEN 5 MG-325 MG -")
            /*var currLine: String = br.readLine
            var count = 0
            while (currLine != null) 
            {
                //medList += tidyOrderName(currLine)
                //not tidying anymore
                medList += currLine
            ...
            br.close()
            fr.close()
            logger.info("added " + count + " order names to array list")*/
            //logger.info(myToString(medList))
            
            //uniquify the medList
            //check cache for matches, send non-matched in cache to R code
                  //med_order <- c(""" + medList.mkString("\"","\",\"","\"") + """)
            logger.info( medList.mkString("\"","\",\"","\""))
            logger.info("starting R processing...")
            val R = org.ddahl.rscala.RClient()
            val result: Array[Array[String]] = R.evalS2("""
              ...
              device_etc_list_onecol <-
                read.delim(
                  "..//utilities//r//medication_mapping//device_etc_list_onecol.txt",
                  header = FALSE,
