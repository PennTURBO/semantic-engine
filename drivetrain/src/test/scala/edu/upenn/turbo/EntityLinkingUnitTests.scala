...
class HealthcareEncounterEntityLinkingUnitTests extends ProjectwideGlobals with FunSuiteLike with BeforeAndAfter with Matchers with EntityLinkingUnitTestFields
{
    ...
    val expectedHealthcareQuery: String = """
      ...
      BIND(IF (BOUND(?OBI_0001929), uri(concat("http://www.itmat.upenn.edu/biobank/",SHA256(CONCAT("?PATO_0000128","localUUID", str(?NCBITaxon_9606))))), ?unbound) AS ?PATO_0000128)
      BIND(IF (BOUND(?VSO_0000006), uri(concat("http://www.itmat.upenn.edu/biobank/",SHA256(CONCAT("?VSO_0000004","localUUID", str(?NCBITaxon_9606))))), ?unbound) AS ?VSO_0000004)
      BIND(uri(concat("http://www.itmat.upenn.edu/biobank/",SHA256(CONCAT("?OBI_0000093","localUUID")))) AS ?OBI_0000093)
      BIND(IF (BOUND(?TURBO_0010138), uri(concat("http://www.itmat.upenn.edu/biobank/",SHA256(CONCAT("?PATO_0000119","localUUID", str(?NCBITaxon_9606))))), ?unbound) AS ?PATO_0000119)
      }
      """
  ...
  class BiobankEncounterEntityLinkingUnitTests extends ProjectwideGlobals with FunSuiteLike with BeforeAndAfter with Matchers with EntityLinkingUnitTestFields
  {
      ...
      val expectedBiobankQuery: String = """
        ...
        BIND(IF (BOUND(?TURBO_0010138), uri(concat("http://www.itmat.upenn.edu/biobank/",SHA256(CONCAT("?PATO_0000119","localUUID", str(?NCBITaxon_9606))))), ?unbound) AS ?PATO_0000119)
        BIND(IF (BOUND(?OBI_0001929), uri(concat("http://www.itmat.upenn.edu/biobank/",SHA256(CONCAT("?PATO_0000128","localUUID", str(?NCBITaxon_9606))))), ?unbound) AS ?PATO_0000128)
        BIND(uri(concat("http://www.itmat.upenn.edu/biobank/",SHA256(CONCAT("?OBI_0000097","localUUID")))) AS ?OBI_0000097)
        }
        """
